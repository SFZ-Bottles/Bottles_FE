...
const getUserId = (children: ReactNode) => {
    ...
    .filter(
        child => isValidElement(child) && child.type === UserIdTypes,
    ).slice(0);
};
...
const getButtons = (children: ReactNode) => {
    ...
    return childrenArray
    .filter(
        child => isValidElement(child) && child.type === ButtonTypes,
    ).slice(0);
};
...
const getProfile = (children: ReactNode) => {
    ...
    return childrenArray
    .filter(
        child => isValidElement(child) && child.type === ProfileTypes,
    ).slice(0,1);
};
...
const getComments = (children: ReactNode) => {
    ...
    return childrenArray
    .filter(
        child => isValidElement(child) && child.type === UserCommentTypes,
    ).slice(0,1);
};
...
const getCreatedTime = (children: ReactNode) => {
    ...
    return childrenArray
    .filter(
        child => isValidElement(child) && child.type === CreatedTimeTypes,
    ).slice(0,1);
};
...
const getMessageImg = (children: ReactNode) => {
    ...
    return childrenArray
    .filter(
        child => isValidElement(child) && child.type === MessageImgTypes,
    ).slice(0,1);
};
...
function CardMain({children, onClick}: IMainProps){
    ...
    return (
        <S.OriginalComment onClick={onClick}>
            {profileContents}
            <S.UserInfo>
                <S.UserId>
                    {userIdContents}
